...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable())
                ...
                .requestMatchers(antMatcher(HttpMethod.POST, "/api/v1/users/add-favorite")).hasRole("USER")
                .requestMatchers(antMatcher(HttpMethod.POST, "/api/v1/users/remove-favorite")).hasRole("USER")
                .requestMatchers(antMatcher(HttpMethod.POST, "/api/v1/users/**")).hasRole("USER")
                .requestMatchers(antMatcher(HttpMethod.GET, "/api/v1/movies")).hasRole("USER")
                .requestMatchers(antMatcher(HttpMethod.PUT, "/api/v1/movies/initial-load")).hasRole("ADMIN")
                .requestMatchers(antMatcher(HttpMethod.PUT, "/api/v1/movies/top-10")).hasRole("USER")
